...
bool dfs(vector< vector<int> > &grafo, bool visitados[], int origen, int destino, bool &found){
    FOR(i,0,grafo[origen].size(),1){
        if(!visitados[grafo[origen][i]]){
            visitados[grafo[origen][i]] = true;
            if(grafo[origen][i] == destino) {
                found = true;
                return true;
            }
            dfs(grafo,visitados,grafo[origen][i],destino, found);
        }
    }
    return false;
}
...
FOR(q,0,queries,1){
    ...
    if(tipo == 1){
        ...
        //preguntamos
        bool visitados[100], found = false;;
        int a,b; cin>>a>>b;
        memset(visitados,false,sizeof(visitados));
        dfs(grafo,visitados,a-1,b-1, found);
